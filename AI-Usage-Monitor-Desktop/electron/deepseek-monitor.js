...
class DeepSeekMonitor {
  ...
  async handlePageLoaded() {
    ...
    if (isLogin) {
      ...
      this._navRetryCount = 0
      this._pageReady = false
      console.log('[DS-Monitor] 未登录，轮询继续运行等待用户登录')
      this.startPolling()  // 仍然启动轮询，用户登录后可自动恢复
      return
    }
    ...
    if (!url.includes('/usage')) {
      if (this._navRetryCount >= 5) { this._navRetryCount = 0; this.startPolling(); return }
      this._navRetryCount++
  ...
  startPolling() {
    ...
    const poll = async () => {
      if (!this.monitorWindow) return
      const url = this.monitorWindow.webContents.getURL()
      // 如果不在用量页（可能是登录页或其他页面），尝试导航过去
      if (!url.includes('/usage')) {
        console.log('[DS-Monitor] 轮询 — 导航到用量页')
        this.monitorWindow.webContents.loadURL(DEEPSEEK_USAGE_URL).catch(() => {})
        return
      }
      console.log('[DS-Monitor] 轮询 — 刷新用量页')
      this.monitorWindow.webContents.loadURL(DEEPSEEK_USAGE_URL).catch(() => {})
    }
    // 立即执行首次轮询，之后按固定间隔执行
    poll()
    this._pollTimer = setInterval(poll, POLL_INTERVAL_MS)
    console.log(`[DS-Monitor] 已启动轮询（每 ${POLL_INTERVAL_MS / 1000}s）`)
  }
